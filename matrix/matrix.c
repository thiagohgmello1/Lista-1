...
**     Method      :    GaussElim
**
**     Description :    Solve LS using Gaussian Elimination with
**                      pivoting.
**     Parameters  :    Matrix pointer and excitation array pointer
...
m_type **GaussElim(m_type **A, m_type *b){
    ...
    m_type m = 0;
    m_type pivot = 0;
    m_type *pos = (m_type *)malloc((row - 1) * sizeof(m_type));
    m_type **newA = (m_type **)malloc((row) * sizeof(m_type*));
    m_type **M = (m_type **)malloc((row) * sizeof(m_type*));

    for (int i = 0; i < row; i++){
        newA[i] = (m_type *)malloc((column) * sizeof(m_type));
    }
    ...
    // Create new matrix with elements in correct order
